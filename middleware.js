...
export default function middleware(request) {
  ...
  const response = Response.redirect(splashUrl, 307);

  // No Max-Age/Expires = a true session cookie. It clears itself when the
  // browser closes, so the splash shows again on their next visit/session
  // rather than being suppressed for a fixed 24 hours.
  response.headers.append('Set-Cookie', 'splash_seen=1; Path=/; SameSite=Lax');
